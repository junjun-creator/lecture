...
public class Test {
	...
	static String shiftString(String word, int shift) {
		...
		for(int i=0;i<wordLength;i++) {
			if(Character.isLowerCase(splitWord[i])) {
				splitWord[i]+=shift;
				if(splitWord[i] > 'z') {
					splitWord[i] = (char) (splitWord[i] - 26);
				}
			}
			else if(Character.isUpperCase(splitWord[i])) {
				splitWord[i]+=shift;
				if(splitWord[i] > 'Z') {
					splitWord[i] = (char) (splitWord[i] - 26);
				}
	...
	static String inputString() {
		...
		do {
			...
			for(int i=0;i<tokenLength;i++) 
				if(!(Character.isLowerCase(token[i])) && !(Character.isUpperCase(token[i])) && !(token[i]==' ')) {
